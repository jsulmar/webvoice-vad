parcelRequire=function(e,r,t,n){var i,o="function"==typeof parcelRequire&&parcelRequire,u="function"==typeof require&&require;function f(t,n){if(!r[t]){if(!e[t]){var i="function"==typeof parcelRequire&&parcelRequire;if(!n&&i)return i(t,!0);if(o)return o(t,!0);if(u&&"string"==typeof t)return u(t);var c=new Error("Cannot find module '"+t+"'");throw c.code="MODULE_NOT_FOUND",c}p.resolve=function(r){return e[t][1][r]||r},p.cache={};var l=r[t]=new f.Module(t);e[t][0].call(l.exports,p,l,l.exports,this)}return r[t].exports;function p(e){return f(p.resolve(e))}}f.isParcelRequire=!0,f.Module=function(e){this.id=e,this.bundle=f,this.exports={}},f.modules=e,f.cache=r,f.parent=o,f.register=function(r,t){e[r]=[function(e,r){r.exports=t},{}]};for(var c=0;c<t.length;c++)try{f(t[c])}catch(e){i||(i=e)}if(t.length){var l=f(t[t.length-1]);"object"==typeof exports&&"undefined"!=typeof module?module.exports=l:"function"==typeof define&&define.amd?define(function(){return l}):n&&(this[n]=l)}if(parcelRequire=f,i)throw i;return f}({"KgAV":[function(require,module,exports) {
"use strict";Object.defineProperty(exports,"__esModule",{value:!0}),exports.default=void 0;class e extends Error{constructor(e){super(e),this.name="WEBVOICESDK_NODE_ERROR",this.error=e}}exports.default=e;
},{}],"h6HF":[function(require,module,exports) {
"use strict";Object.defineProperty(exports,"__esModule",{value:!0}),exports.default=void 0;var t=e(require("../nodes/error.js"));function e(t){return t&&t.__esModule?t:{default:t}}class s extends EventTarget{constructor(){super(),this.status="non-emitting"}async start(e){if(this.hookedOn)throw new t.default("node ".concat(this.type," is already hooked on ").concat(this.hookedOn,", call stop() first"));if(!e)throw new t.default("".concat(this.type," requires a node argument to hook on"));if(e&&!this.hookableOnNodeTypes.includes(e.type))throw new t.default("".concat(this.type," node cannot hook on ").concat(e.type));this.startWorker(),this.hookedOn=e,this.resume()}startWorker(){this.worker&&(this.workerRuntime=this.worker.init(),this.workerRuntime.onmessage=(t=>{this.dispatchEvent(new CustomEvent(this.event,{detail:t.data}))}))}stop(){this.pause(),this.workerRuntime&&(this.workerRuntime.terminate(),delete this.workerRuntime),this.hookedOn=null}pause(){this.hookedOn&&"emitting"==this.status&&("mic"!=this.type&&this.hookedOn.removeEventListener(this.hookedOn.event,this.handler),this.status="non-emitting")}resume(){this.hookedOn&&"non-emitting"==this.status&&("mic"!=this.type&&this.hookedOn.addEventListener(this.hookedOn.event,this.handler),this.status="emitting")}}exports.default=s;
},{"../nodes/error.js":"KgAV"}],"Vg1w":[function(require,module,exports) {
"use strict";Object.defineProperty(exports,"__esModule",{value:!0}),exports.default=void 0;var t=s(require("./node.js")),e=s(require("./error.js"));function s(t){return t&&t.__esModule?t:{default:t}}function o(t,e){var s=Object.keys(t);if(Object.getOwnPropertySymbols){var o=Object.getOwnPropertySymbols(t);e&&(o=o.filter(function(e){return Object.getOwnPropertyDescriptor(t,e).enumerable})),s.push.apply(s,o)}return s}function i(t){for(var e=1;e<arguments.length;e++){var s=null!=arguments[e]?arguments[e]:{};e%2?o(Object(s),!0).forEach(function(e){r(t,e,s[e])}):Object.getOwnPropertyDescriptors?Object.defineProperties(t,Object.getOwnPropertyDescriptors(s)):o(Object(s)).forEach(function(e){Object.defineProperty(t,e,Object.getOwnPropertyDescriptor(s,e))})}return t}function r(t,e,s){return e in t?Object.defineProperty(t,e,{value:s,enumerable:!0,configurable:!0,writable:!0}):t[e]=s,t}class n extends t.default{constructor(){let t=arguments.length>0&&void 0!==arguments[0]?arguments[0]:{};super(),this.options=i(i({},n.defaultOptions),t),this.hookableOnNodeTypes=[],this.type="mic",this.event="srcFrame",this.hookedOn=null}async start(){let t=arguments.length>0&&void 0!==arguments[0]?arguments[0]:{};if(null==this||!this.hookedOn){if(this.options=i(i({},this.options),t),this.hookedOn)throw new e.default("Source is already started, call stop() first");const{stream:s,context:o,node:r}=this.options.source;this.stream=s||await navigator.mediaDevices.getUserMedia({audio:this.options.constraints,video:!1}),s&&o?this.context=o:(this.context=new(window.AudioContext||window.webkitAudioContext),this.localContext=this.context),this.mediaStreamSource=s&&o&&r?r:this.context.createMediaStreamSource(this.stream),this.hookedOn=!0,this.sampleRate=this.context.sampleRate,this.scriptProcessor=this.context.createScriptProcessor(this.frameSize,1,1),"non-emitting"==this.status&&this.hookedOn&&(this.scriptProcessor.onaudioprocess=(t=>{const e=t.inputBuffer.getChannelData(0);this.dispatchEvent(new CustomEvent(this.event,{detail:e})),this.options.onAudioFrame(e)}),this.mediaStreamSource.connect(this.scriptProcessor),this.scriptProcessor.connect(this.context.destination),this.status="emitting")}return Promise.resolve()}resume(){super.resume(),this.mediaStreamSource.connect(this.scriptProcessor),this.scriptProcessor.connect(this.context.destination)}pause(){super.pause(),this.mediaStreamSource.disconnect(),this.scriptProcessor.disconnect()}async stop(){this.hookedOn&&(this.stream.getTracks().map(t=>"live"===t.readyState&&"audio"===t.kind&&t.stop()),this.pause(),delete this.mediaStreamSource,delete this.scriptProcessor,null!=this&&this.localContext&&(console.log("Src closing local context."),await this.localContext.close(),delete this.localContext),delete this.stream,delete this.context,this.hookedOn=null)}}exports.default=n,r(n,"defaultOptions",{source:{stream:null,context:null,node:null},onAudioFrame:()=>{},frameSize:4096,constraints:{echoCancellation:!0,autoGainControl:!0,noiseSuppression:!0,channelCount:1}});
},{"./node.js":"h6HF","./error.js":"KgAV"}],"F24j":[function(require,module,exports) {
"use strict";Object.defineProperty(exports,"__esModule",{value:!0}),exports.default=void 0;var e=t(require("./src"));function t(e){return e&&e.__esModule?e:{default:e}}function r(e,t){var r=Object.keys(e);if(Object.getOwnPropertySymbols){var n=Object.getOwnPropertySymbols(e);t&&(n=n.filter(function(t){return Object.getOwnPropertyDescriptor(e,t).enumerable})),r.push.apply(r,n)}return r}function n(e){for(var t=1;t<arguments.length;t++){var n=null!=arguments[t]?arguments[t]:{};t%2?r(Object(n),!0).forEach(function(t){o(e,t,n[t])}):Object.getOwnPropertyDescriptors?Object.defineProperties(e,Object.getOwnPropertyDescriptors(n)):r(Object(n)).forEach(function(t){Object.defineProperty(e,t,Object.getOwnPropertyDescriptor(n,t))})}return e}function o(e,t,r){return t in e?Object.defineProperty(e,t,{value:r,enumerable:!0,configurable:!0,writable:!0}):e[t]=r,e}class s extends e.default{constructor(){let e=arguments.length>0&&void 0!==arguments[0]?arguments[0]:{};super(n(n({},s.defaultOptions),e))}async start(){return await super.start(),Promise.resolve()}resume(){super.resume()}pause(){super.pause()}stop(){super.stop()}}exports.default=s,o(s,"defaultOptions",{frameSize:4096,constraints:{echoCancellation:!0,autoGainControl:!0,noiseSuppression:!0,channelCount:1}});
},{"./src":"Vg1w"}],"tqbH":[function(require,module,exports) {

    const encodedWasm = ""
    const binaryWasmString = atob(encodedWasm)
    const buf = new Uint8Array(binaryWasmString.length)
    for (var i = 0; i < binaryWasmString.length; i++) {
      buf[i] = binaryWasmString.charCodeAt(i);
    }

    module.exports.forInstanciate = function() {
      const blob = new Blob([buf], { type: 'application/wasm' });
      const wasmUrl = URL.createObjectURL(blob);
      return wasmUrl
    }

    module.exports.forInstanciateStreaming = new Promise(resolve => {
      const fetchableWasm = new Response([buf], { status: 200, headers: { "Content-Type": "application/wasm" } })
      resolve(fetchableWasm)
    })
    
},{}],"pBGv":[function(require,module,exports) {

var t,e,n=module.exports={};function r(){throw new Error("setTimeout has not been defined")}function o(){throw new Error("clearTimeout has not been defined")}function i(e){if(t===setTimeout)return setTimeout(e,0);if((t===r||!t)&&setTimeout)return t=setTimeout,setTimeout(e,0);try{return t(e,0)}catch(n){try{return t.call(null,e,0)}catch(n){return t.call(this,e,0)}}}function u(t){if(e===clearTimeout)return clearTimeout(t);if((e===o||!e)&&clearTimeout)return e=clearTimeout,clearTimeout(t);try{return e(t)}catch(n){try{return e.call(null,t)}catch(n){return e.call(this,t)}}}!function(){try{t="function"==typeof setTimeout?setTimeout:r}catch(n){t=r}try{e="function"==typeof clearTimeout?clearTimeout:o}catch(n){e=o}}();var c,s=[],l=!1,a=-1;function f(){l&&c&&(l=!1,c.length?s=c.concat(s):a=-1,s.length&&h())}function h(){if(!l){var t=i(f);l=!0;for(var e=s.length;e;){for(c=s,s=[];++a<e;)c&&c[a].run();a=-1,e=s.length}c=null,l=!1,u(t)}}function m(t,e){this.fun=t,this.array=e}function p(){}n.nextTick=function(t){var e=new Array(arguments.length-1);if(arguments.length>1)for(var n=1;n<arguments.length;n++)e[n-1]=arguments[n];s.push(new m(t,e)),1!==s.length||l||i(h)},m.prototype.run=function(){this.fun.apply(null,this.array)},n.title="browser",n.env={},n.argv=[],n.version="",n.versions={},n.on=p,n.addListener=p,n.once=p,n.off=p,n.removeListener=p,n.removeAllListeners=p,n.emit=p,n.prependListener=p,n.prependOnceListener=p,n.listeners=function(t){return[]},n.binding=function(t){throw new Error("process.binding is not supported")},n.cwd=function(){return"/"},n.chdir=function(t){throw new Error("process.chdir is not supported")},n.umask=function(){return 0};
},{}],"sNYT":[function(require,module,exports) {
var process = require("process");
var n=require("process");Object.defineProperty(exports,"__esModule",{value:!0}),exports.default=void 0;var e=r(require("../../vendor/rnnoise-wasm/dist/rnnoise.wasm"));function t(n){if("function"!=typeof WeakMap)return null;var e=new WeakMap,r=new WeakMap;return(t=function(n){return n?r:e})(n)}function r(n,e){if(!e&&n&&n.__esModule)return n;if(null===n||"object"!=typeof n&&"function"!=typeof n)return{default:n};var r=t(e);if(r&&r.has(n))return r.get(n);var o={},i=Object.defineProperty&&Object.getOwnPropertyDescriptor;for(var a in n)if("default"!==a&&Object.prototype.hasOwnProperty.call(n,a)){var u=i?Object.getOwnPropertyDescriptor(n,a):null;u&&(u.get||u.set)?Object.defineProperty(o,a,u):o[a]=n[a]}return o.default=n,r&&r.set(n,o),o}var o=function(){var t="undefined"!=typeof document&&document.currentScript?document.currentScript.src:void 0;return function(r){var o;(r=void 0!==(r=r||{})?r:{}).ready=new Promise(function(n,e){o=n,e});var i,a={};for(i in r)r.hasOwnProperty(i)&&(a[i]=r[i]);var u,s=[],f=!1,c=!1;f="object"==typeof window,c="function"==typeof importScripts,u="object"==typeof n&&"object"==typeof n.versions&&"string"==typeof n.versions.node;var l,p="";(f||c)&&(c?p=self.location.href:document.currentScript&&(p=document.currentScript.src),t&&(p=t),p=0!==p.indexOf("blob:")?p.substr(0,p.lastIndexOf("/")+1):"",function(n){var e=new XMLHttpRequest;return e.open("GET",n,!1),e.send(null),e.responseText},c&&(l=function(n){var e=new XMLHttpRequest;return e.open("GET",n,!1),e.responseType="arraybuffer",e.send(null),new Uint8Array(e.response)}),function(n,e,t){var r=new XMLHttpRequest;r.open("GET",n,!0),r.responseType="arraybuffer",r.onload=function(){200==r.status||0==r.status&&r.response?e(r.response):t()},r.onerror=t,r.send(null)});var y,m,d=r.print||console.log.bind(console),b=r.printErr||console.warn.bind(console);for(i in a)a.hasOwnProperty(i)&&(r[i]=a[i]);a=null,r.arguments&&(s=r.arguments),r.thisProgram&&r.thisProgram,r.quit&&r.quit,r.wasmBinary&&(y=r.wasmBinary),r.noExitRuntime&&r.noExitRuntime,"object"!=typeof WebAssembly&&b("no native wasm support detected");var w,h,v,_,g=new WebAssembly.Table({initial:1,maximum:1,element:"anyfunc"}),A=!1;function R(n){w=n,r.HEAP8=new Int8Array(n),r.HEAP16=new Int16Array(n),r.HEAP32=v=new Int32Array(n),r.HEAPU8=h=new Uint8Array(n),r.HEAPU16=new Uint16Array(n),r.HEAPU32=new Uint32Array(n),r.HEAPF32=_=new Float32Array(n),r.HEAPF64=new Float64Array(n),"object"==typeof n&&"object"==typeof _||console.log("updateGlobalBufferAndViews buf, HEAPF32:",typeof n,typeof _)}var P=r.INITIAL_MEMORY||16777216;function I(n){for(;n.length>0;){var e=n.shift();if("function"!=typeof e){var t=e.func;"number"==typeof t?void 0===e.arg?r.dynCall_v(t):r.dynCall_vi(t,e.arg):t(void 0===e.arg?null:e.arg)}else e(r)}}(m=r.wasmMemory?r.wasmMemory:new WebAssembly.Memory({initial:P/65536,maximum:32768}))&&(w=m.buffer),P=w.byteLength,R(w),v[24364]=5340496;var E=[],M=[],O=[],W=[];var j=0,S=null,x=null;function H(n){throw r.onAbort&&r.onAbort(n),d(n+=""),b(n),A=!0,1,n="abort("+n+"). Build with -s ASSERTIONS=1 for more info.",new WebAssembly.RuntimeError(n)}r.preloadedImages={},r.preloadedAudios={};var T=e.forInstanciate();function U(){try{if(y)return new Uint8Array(y);if(l)return l(T);throw"both async and sync fetching of the wasm failed"}catch(b){H(b)}}function q(n){try{return m.grow(n-w.byteLength+65535>>>16),R(m.buffer),1}catch(e){}}M.push({func:function(){D()}});var k,B={a:function(n,e,t){h.copyWithin(n,e,e+t)},b:function(n){n>>>=0;var e=h.length;if(n>2147483648)return!1;for(var t,r,o=1;o<=4;o*=2){var i=e*(1+.2/o);if(i=Math.min(i,n+100663296),q(Math.min(2147483648,((t=Math.max(16777216,n,i))%(r=65536)>0&&(t+=r-t%r),t))))return!0}return!1},memory:m,table:g},D=(function(){var n={a:B};function e(n,e){var t=n.exports;r.asm=t,function(n){if(j--,r.monitorRunDependencies&&r.monitorRunDependencies(j),0==j&&(null!==S&&(clearInterval(S),S=null),x)){var e=x;x=null,e()}}()}function t(n){e(n.instance)}function o(e){return(y||!f&&!c||"function"!=typeof fetch?new Promise(function(n,e){n(U())}):fetch(T,{credentials:"same-origin"}).then(function(n){if(!n.ok)throw"failed to load wasm binary file at '"+T+"'";return n.arrayBuffer()}).catch(function(){return U()})).then(function(e){return console.log(e),WebAssembly.instantiate(e,n)}).then(e,function(n){b("failed to asynchronously prepare wasm: "+n),H(n)})}if(j++,r.monitorRunDependencies&&r.monitorRunDependencies(j),r.instantiateWasm)try{return r.instantiateWasm(n,e)}catch(i){return b("Module.instantiateWasm callback failed with error: "+i),!1}(function(){if(y||"function"!=typeof WebAssembly.instantiateStreaming||"function"!=typeof fetch)return o(t);fetch(T,{credentials:"same-origin"}).then(function(e){return WebAssembly.instantiateStreaming(e,n).then(t,function(n){return b("wasm streaming compile failed: "+n),b("falling back to ArrayBuffer instantiation"),o(t)})})})()}(),r.___wasm_call_ctors=function(){return(D=r.___wasm_call_ctors=r.asm.c).apply(null,arguments)});r._rnnoise_init=function(){return(r._rnnoise_init=r.asm.d).apply(null,arguments)},r._rnnoise_create=function(){return(r._rnnoise_create=r.asm.e).apply(null,arguments)},r._malloc=function(){return(r._malloc=r.asm.f).apply(null,arguments)},r._rnnoise_destroy=function(){return(r._rnnoise_destroy=r.asm.g).apply(null,arguments)},r._free=function(){return(r._free=r.asm.h).apply(null,arguments)},r._rnnoise_process_frame=function(){return(r._rnnoise_process_frame=r.asm.i).apply(null,arguments)};function L(n){function e(){k||(k=!0,r.calledRun=!0,A||(!0,I(M),I(O),o(r),r.onRuntimeInitialized&&r.onRuntimeInitialized(),function(){if(r.postRun)for("function"==typeof r.postRun&&(r.postRun=[r.postRun]);r.postRun.length;)n=r.postRun.shift(),W.unshift(n);var n;I(W)}()))}n=n||s,j>0||(!function(){if(r.preRun)for("function"==typeof r.preRun&&(r.preRun=[r.preRun]);r.preRun.length;)n=r.preRun.shift(),E.unshift(n);var n;I(E)}(),j>0||(r.setStatus?(r.setStatus("Running..."),setTimeout(function(){setTimeout(function(){r.setStatus("")},1),e()},1)):e()))}if(x=function n(){k||L(),k||(x=n)},r.run=L,r.preInit)for("function"==typeof r.preInit&&(r.preInit=[r.preInit]);r.preInit.length>0;)r.preInit.pop()();return!0,L(),r.ready}}(),i=o;exports.default=i;
},{"../../vendor/rnnoise-wasm/dist/rnnoise.wasm":"tqbH","process":"pBGv"}],"AsRh":[function(require,module,exports) {
"use strict";Object.defineProperty(exports,"__esModule",{value:!0}),exports.Rnnoise=exports.RNNOISE_SAMPLE_LENGTH=void 0;var t=e(require("./loader"));function e(t){return t&&t.__esModule?t:{default:t}}const s=480;exports.RNNOISE_SAMPLE_LENGTH=s;const r=4*s;class a{constructor(){this.destroyed=!1,this.init()}async init(){try{if(this.wasmInterface=await(0,t.default)(),this.wasmPcmInput=this.wasmInterface._malloc(r),!this.wasmPcmInput)throw Error("Failed to create wasm input memory buffer");if(this.wasmPcmOutput=this.wasmInterface._malloc(r),!this.wasmPcmOutput)throw this.wasmInterface._free(this.wasmPcmInput),Error("Failed to create wasm output memory buffer");this.wasmPcmInput&&(this.wasmPcmInputF32Index=this.wasmPcmInput/4),this.context=this.wasmInterface._rnnoise_create()}catch(e){throw this.releaseWasmResources(),e}return this}releaseWasmResources(){this.wasmPcmInput&&(this.wasmInterface._free(this.wasmPcmInput),this.wasmPcmInput=null),this.wasmPcmOutput&&(this.wasmInterface._free(this.wasmPcmOutput),this.wasmPcmOutput=null),this.context&&(this.wasmInterface._rnnoise_destroy(this.context),this.context=null)}calculateAudioFrameVAD(t){if(this.destroyed)throw new Error("RnnoiseProcessor instance is destroyed, please create another one!");const e=t.length;if(e!==s)throw new Error("Rnnoise can only process PCM frames of 480 samples! Input sample was:".concat(e));return this.convertTo16BitPCM(t),this.copyPCMSampleToWasmBuffer(t),this.wasmInterface,this.wasmInterface?this.wasmInterface._rnnoise_process_frame(this.context,this.wasmPcmOutput,this.wasmPcmInput):0}convertTo16BitPCM(t){t.forEach((e,s)=>{t[s]=32767*e})}copyPCMSampleToWasmBuffer(t){var e;"object"==typeof(null===(e=this.wasmInterface)||void 0===e?void 0:e.HEAPF32)&&this.wasmInterface.HEAPF32.set(t,this.wasmPcmInputF32Index)}destroy(){this.destroyed||(this.releaseWasmResources(),this.destroyed=!0)}}exports.Rnnoise=a;
},{"./loader":"sNYT"}],"SsLo":[function(require,module,exports) {
"use strict";Object.defineProperty(exports,"__esModule",{value:!0}),exports.default=void 0;var t=i(require("../nodes/node.js")),e=require("../rnnoise/index.js");function i(t){return t&&t.__esModule?t:{default:t}}function s(t,e,i){return e in t?Object.defineProperty(t,e,{value:i,enumerable:!0,configurable:!0,writable:!0}):t[e]=i,t}let o=0,n=0,h=0;const a=t=>{h=new Date,console.log("------------\x3e event ".concat(t," (elapsed ").concat(h-n,")")),n=h},r=function(t){const i=t.detail,s=[...this.bufferResidue,...i];let n=0;for(;n+e.RNNOISE_SAMPLE_LENGTH<s.length;n+=e.RNNOISE_SAMPLE_LENGTH){const t=s.slice(n,n+e.RNNOISE_SAMPLE_LENGTH),i=this.wasmRuntime.calculateAudioFrameVAD(t);this.activations.length==this.options.numActivations&&this.activations.shift(),this.activations.push(0+(i>this.options.threshold)),this.options.dlog&&!(o++%10)&&console.log("vadScore:",i.toFixed(3));let h=this.activations.reduce((t,e)=>t+e);i>=this.options.threshold&&this.redemptionTimer&&(clearTimeout(this.redemptionTimer),this.redemptionTimer=!1),i<this.options.threshold&&!this.redemptionTimer&&this.speaking&&(this.redemptionTimer=setTimeout(()=>{this.wasmRuntime&&(this.speaking=!1,this.options.dlog&&a(this.speaking),this.dispatchEvent(new CustomEvent(this.event,{detail:!1})))},this.options.timeAfterStop)),h>=this.options.numActivations&&!this.speaking&&(this.speaking=!0,this.options.dlog&&a(this.speaking),this.dispatchEvent(new CustomEvent(this.event,{detail:!0})))}this.bufferResidue=s.slice(n,s.length)};class d extends t.default{constructor(){let{numActivations:t=10,threshold:e=.85,timeAfterStop:i=800,dlog:s=!1}=arguments.length>0&&void 0!==arguments[0]?arguments[0]:{};console.log("vad constructor dlog arg=".concat(s)),super(),this.handler=r.bind(this),this.type="vad",this.event="speakingStatus",this.hookableOnNodeTypes=["mic","downSampler","speechPreemphaser"],this.options={numActivations:t,threshold:e,timeAfterStop:i,dlog:s},this.options.dlog&&console.log("vad options::",this.options)}start(t){this.wasmRuntime=new e.Rnnoise,this.bufferResidue=new Float32Array([]),this.redemptionTimer=!1,this.activations=[],this.speaking=!1,super.start(t)}stop(){this.hookedOn&&(super.stop(),this.wasmRuntime.destroy(),delete this.wasmRuntime)}}exports.default=d,s(d,"defaultOptions",{numActivations:10,threshold:.85,timeAfterStop:800,dlog:!1});
},{"../nodes/node.js":"h6HF","../rnnoise/index.js":"AsRh"}],"Z7GA":[function(require,module,exports) {
"use strict";var e=r(require("./webvoicesdk/nodes/mic.js")),d=r(require("./webvoicesdk/nodes/src.js")),s=r(require("./webvoicesdk/nodes/vad.js"));function r(e){return e&&e.__esModule?e:{default:e}}const o={Src:d.default,Mic:e.default,Vad:s.default};window.vad=o,module.exports=o;
},{"./webvoicesdk/nodes/mic.js":"F24j","./webvoicesdk/nodes/src.js":"Vg1w","./webvoicesdk/nodes/vad.js":"SsLo"}]},{},["Z7GA"], null)